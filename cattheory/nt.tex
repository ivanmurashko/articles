...
\mynameref{def:functor}s $F: \cat{C} \Rightarrow \cat{D}$ and $G:
\cat{C} \Rightarrow \cat{D}$. If we have an \mynameref{def:object} $a \in \catob{C}$ then
...
set of \mynameref{def:morphism}s $\alpha \subset \cathom{D}$ that
satisfied the following conditions:
\begin{itemize}
\item For every \mynameref{def:object} $a \in \catob{C}$ $\exists
\alpha_a \in \hom\left(F(a), G(a)\right)$ - \mynameref{def:morphism}
...
functors $F$ and $G$: $\alpha: F \xrightarrow{.} G$. 
\end{definition}
...
the composition $\beta_a \cdot \alpha_a$ i.e. we can define the
composition of natural transformations $\alpha, \beta$ as 
\(
\beta \cdot \alpha = \left\{
\beta_a \circ \alpha_a | a \in \catob{C}
\right\}
\), note that we use $\cdot$ and not $\circ$ for the composition
definition. 
...
morphism mapping. We will require that all morphisms (see
\cref{fig:nt_morphism_mapping_composition}) commutes. The composition
defined in the such way is called
...
Also we have $\alpha : F \xrightarrow{.} G, \beta: G \xrightarrow{.}
H$ - natural transformations. We can compose the $\alpha$ and $\beta$
as follows 
\[
\alpha \cdot \beta: F \xrightarrow{.} H.
\]
This composition
...
\cat{D}$ and another one $J,K: \cat{D} \Rightarrow \cat{E}$. If we have a
natural transformation between each pair: $\alpha : F \xrightarrow{.}
G$ for the first one and $\beta : J \xrightarrow{.}
K$ for the second one. We can create a new transformation
\[
\alpha \circ \beta: F \circ J \xrightarrow{.} G \circ K
\] 
that is called \textit{horizontal composition}.
\end{definition}

\begin{definition}[Left composition]
\label{def:lc}
If we have 3 categories $\cat{C}, \cat{D}, \cat{E}$, 
\mynameref{def:functor}s $F,G: \cat{C} \Rightarrow \cat{D}$, $H: \cat{D} \to
\cat{E}$ and \mynameref{def:nt} 
$\alpha: F \xrightarrow{.} G$ then we can construct a new natural
transformations:
\[
H \alpha : H \circ F \xrightarrow{.} H \circ G
\]
that is called \textit{left composition} of functor and natural
transformation. 
\footnote{The definition is my own definition and it has to be relaced
with the correct one TBD}
\end{definition}

\begin{definition}[Left whiskering]
\label{def:lw}
If we have 3 categories $\cat{B}, \cat{C}, \cat{D}$, 
\mynameref{def:functor}s $F,G: \cat{C} \Rightarrow \cat{D}$, $H: \cat{B} \to
\cat{C}$ and \mynameref{def:nt} 
$\alpha: F \xrightarrow{.} G$ then we can construct a new natural
transformations:
\[
\alpha H : F \circ H \xrightarrow{.} G \circ H
\]
that is called \textit{left whiskering} of functor and natural
transformation \cite{misc:whiskering}. 
\end{definition}
...
In Haskell the most functions are \mynameref{def:pp_function}s 
