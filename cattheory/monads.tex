...
(x \cdot y) \cdot z
\]
i.e. the corresponding morphisms commute:
\[
\mu\circ\left(\mu \times \idm{M}\right) =
\mu \circ \left(\idm{M} \times \mu\right) \circ \alpha.
\]
This corresponding diagram is shown in \cref{fig:monoid_mu_alpha}.
...
$I = \{0\}$ to a special element $e \in M$ such that
...
\idm{M}: M \to M.
\]
The 2 morphisms were combined into one using
\mynameref{def:product_of_morphisms} for instance:
\[
...
product (see \cref{ex:product_bifunctor}). There is a
...
\mynameref{def:hilbert_space}s. Note that the monoidal product is
\mynameref{def:bifunctor}. 
\end{remark}
...
\eqref{eq:monoidalobject1} and \eqref{eq:monoid_def} then we can say
...
For the identity element consider the same trick and replace 
TBD
