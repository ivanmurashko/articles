...
\mynameref{def:functor}s and especially by \mynameref{def:bifunctor}s. 
...
\mynameref{def:singleton} $t \in \catob{C}$. 
The object $m \in
...
In the \mynameref{def:setcategory} we have
\mynameref{def:cartesian_product} as monoidal product. Note that there
is also the \mynameref{def:product} in the category. There is a
