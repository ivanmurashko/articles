...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Functor $F: \cat{C} \Rightarrow \cat{D}$ definition}
  \label{fig:functor}
\end{figure}
\end{definition}
...
\mynameref{def:functor} $E: \cat{C} \Rightarrow \cat{C}$ i.e. 
...
\label{def:idfunctor}
Let $\cat{C}$ is a \mynameref{def:category}. The
\mynameref{def:functor} $Id_C: \cat{C} \Rightarrow \cat{C}$ is called \textit{identity
...
\mynameref{def:functor} $\cat{C^{op}} \Rightarrow \cat{D}$ is called
...
$\cat{C^{op}} \times \cat{C} \Rightarrow \cat{C}$ is called
\textit{profunctor}. 
\end{definition}
