...
In the quantum mechanics we say about Hilber spaces. The
transformation between them that preserves the structure is called
linear map or linear transformations.
\begin{definition}[Linear map]
\label{def:linear_map}
The linear map between2 Hilber spaces $A$ and $B$ is a mapping $f: A
\to B$ that preserves additions 
\[
...
\]
where $a,a_{1,2} \in A$ and $f(a), f(a_{1,2}) \in B$.
\end{definition}
...
\begin{example}[\textbf{FdHilb} category]
  ...
  Hilber spaces and \mynameref{def:morphism}s are
  \mynameref{def:linear_map}s. The category is denoted as
  \textbf{FdHilb}. It is very similar to   \mynameref{ex:relcategory}.
  The brief relation is   describe in the
  ...
  \begin{table}
    ...
    \begin{tabular}{l|l|l|l}
      ...
      \midrule
      \mynameref{def:object} & \mynameref{def:set} & \mynameref{def:set} & finite
      dimensional Hilber space\\
      \mynameref{def:morphism} & \mynameref{def:function} & \mynameref{def:binary_relation} &
