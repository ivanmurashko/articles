...
\index{Category!dual}
If $C$ is a \mynameref{def:category} then oppsite (or dual) category
$C^{op}$ is constructed in the following way: \mynameref{def:object}s
are the same, but the \mynameref{def:morphism}s are inverted i.e. 
if $f \in \mathrm{hom}(C)$ and $\dom f = a, \cod f = b$, then the
corresponding morphism $f^{op} \in \mathrm{hom}(C^{op})$ has $\dom f^{op} =
...
\begin{figure}
  ...
  \caption{Opposite category $C^{op}$ to the category from
    ...
    $\mathrm{ob}(C^{op}) = \mathrm{ob}(C) = \{a,b,c,d\}$ and 7 morphisms
    $\mathrm{hom}(C^{op}) = \{f^{op},g^{op},h^{op} = g^{op} \circ
...
\mathrm{hom}(C)$ then $f \circ g$ translated into $g^{op} \circ
