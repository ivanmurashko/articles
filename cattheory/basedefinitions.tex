...
\begin{remark}[Class of Objects]
  ...
  $\mathrm{ob}(\cat{C})$ (see \cref{fig:class_of_objects}).
\end{remark}
...
\begin{figure}
  ...
  \end{tikzpicture}
  \caption{Class of objects $\mathrm{ob}(\cat{C})=\{a,b,c,d\}$}
...
\begin{remark}[Class of Morphisms]
  ...
  $\mathrm{hom}(\cat{C})$ (see \cref{fig:class_of_morphisms})
\end{remark}
...
\begin{definition}[Category]
  ...
  \item \mynameref{def:class} of
    \mynameref{def:object}s $\mathrm{ob}(\cat{C})$
  \item \mynameref{def:class} of \mynameref{def:morphism}s $\mathrm{hom}(\cat{C})$
    defined for $\mathrm{ob}(\cat{C})$, i.e. each morphism $f_{ab}$ from 
    $\mathrm{hom}(\cat{C})$ has both source
    $a$ and target $b$ from $\mathrm{ob}(\cat{C})$
  \end{itemize}
...
\begin{figure}
  ...
  \caption{Category $\cat{C}$. It consists of 4 objects
    $\mathrm{ob}(\cat{C}) = \{a,b,c,d\}$ and 7 morphisms
    $\mathrm{ob}(\cat{C}) = \{f,g,h = f \circ g, \idarrow[a], \idarrow[b],
    \idarrow[c], \idarrow[d]\}$}
  \label{fig:category}
\end{figure}
...
if $f \in \mathrm{hom}(\cat{C})$ and $\dom f = a, \cod f = b$, then the
corresponding morphism $f^{op} \in \mathrm{hom}(\cat{C}^{op})$ has $\dom f^{op} =
...
\begin{figure}
  ...
  \caption{Opposite category $C^{op}$ to the category from
    ...
    $\mathrm{ob}(\cat{C}^{op}) = \mathrm{ob}(\cat{C}) = \{a,b,c,d\}$ and 7 morphisms
    $\mathrm{hom}(\cat{C}^{op}) = \{f^{op},g^{op},h^{op} = g^{op} \circ
...
\mathrm{hom}(\cat{C})$ then $f \circ g$ translated into $g^{op} \circ
