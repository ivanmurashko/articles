...
\begin{example}[\textbf{FdHilb} category]
  ...
  \begin{table}
    ...
    \begin{tabular}{l|l|l|l}
      ...
      \midrule
      \mynameref{def:object} & \mynameref{def:set} & \mynameref{def:set} & \makecell{finite
      ...
      \mynameref{def:initial_object} & empty set & empty set & \makecell{trivial
      Hilber space\\ of dimensional 0} \\
...
\begin{example}[Rabi oscillations]  
  ...
  Hilber space i.e. have only one \mynameref{def:object}. Lets call
  it as $\ket{\psi}$. The category will be called as
  $\cat{\mathcal{H}_2}$. I.e. $\catob{\mathcal{H}_2} =
  \{\ket{\psi}\}$.  
...
\begin{figure}
  ...
  \begin{tikzpicture}[ele/.style={fill=black,circle,minimum
    ...
    \draw (0) to [out=45,in=135,looseness=200] node[pos=0.5,above]
          {$\idarrow[\ket{\psi}] = \hat{L}^4$} (0);

  \end{tikzpicture}
  \caption{Rabi oscillations as a category $\cat{\mathcal{H}_2}$}
