...
The
transformation between 2 \mynameref{def:hilber_space}s that preserves
the structure is called 
linear map or linear transformations.
\begin{definition}[Linear map]
\label{def:linear_map}
The linear map between2 \mynameref{def:hilbert_space}s $\mathcal{H}_n$
and $\mathcal{H}_m$ is a mapping $f: \mathcal{H}_n 
\to \mathcal{H}_m$ that preserves additions 
\[
...
\]
where $a,a_{1,2} \in \mathcal{H}_n$ and $f(a), f(a_{1,2}) \in \mathcal{H}_m$.
\end{definition}
...
\begin{example}[\textbf{FdHilb} category]
  ...
  \begin{table}
    ...
    \begin{tabular}{l|l|l|l}
      ...
      dimensional \\\mynameref{def:hilbert_space}}\\
...
\begin{example}[Rabi oscillations]  
  ...
  \mynameref{def:hilbert_space} i.e. have only one \mynameref{def:object}. Lets call
