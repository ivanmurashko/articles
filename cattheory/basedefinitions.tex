...
\mynameref{def:morphism}s. The languages as Haskell can be considered
(with some limitations see \mynameref{rem:hask_lazy_eval}) as a
category in whole i.e. all possible constructions that can be created by the
language tools can be considered as a huge category. Another languages
such as C++ assumes side effects and there can be 
easy constructed objects that does not satisfied the axioms of
category theory. The last language Scala is something between.
...
principle. The \mynameref{ex:setcategory} cannot be used for it
because it does not satisfied the principle. 

A simple modification of the \textbf{Set} category can satisfy the
principle. 
\begin{example}[\textbf{Rel} category]
  \label{ex:relcategory}
  \index{Object!\textbf{Rel} example}
  \index{Morphism!\textbf{Rel} example}
  We will consider a set of sets (same as \mynameref{ex:setcategory})
  ...
  \mynameref{ex:setcategory}) but there can be a situation when $Y'$
...
\begin{example}[\textbf{FdHilb} category]
  ...
  We are going to consider finite dimensional Hilber space as a
  category where \mynameref{def:object}s are the collections
  (\mynameref{def:set}s) of system
  states in the Hilbert space and \mynameref{def:morphism}s are
  linear operators which transforms one state into another one. 

%%   For our example we consider a 2 level atom with states $\ket{a}$ -
%%   excited and $\ket{b}$. The atom interacts with light beam of
%%   frequency $\omega = \omega_{ab}$. The interaction time $t$ is fixed and
%%   corresponds to $\omega_R t = \pi$ where $\omega_R$ - Rabi
%%   frequency \cite{bib:quantum_optics_mine}. The state of the system is
...
%%     \node[ele,label=left:$\ket{\psi}_0$] (0) at (0,0) {};    
%%     \node[ele,label=left:$\ket{\psi}_1$] (1) at (0,2) {};    
%%     \node[ele,label=right:$\ket{\psi}_2$] (2) at (2,2) {};
%%     \node[ele,label=right:$\ket{\psi}_3$] (3) at (2,0) {};
    
%%     \draw[->,thick,shorten <=2pt,shorten >=2pt] (0) to
%%     node[pos=0.5,sloped,above]{$\hat{L}_{01}$} (1); 
%%     \draw[->,thick,shorten <=2pt,shorten >=2pt] (1) to
%%     node[pos=0.5,sloped,above]{$\hat{L}_{12}$} (2); 
%%     \draw[->,thick,shorten <=2pt,shorten >=2pt] (2) to
%%     node[pos=0.5,sloped,above]{$\hat{L}_{23}$} (3); 
%%     \draw[->,thick,shorten <=2pt,shorten >=2pt] (3) to
%%     node[pos=0.5,sloped,below]{$\hat{L}_{30}$} (0); 

%%     \draw (0) to [out=-45,in=-135,looseness=20] node[pos=0.5,left] {$\idarrow[0]$} (0);
%%     \draw (1) to [out=45,in=135,looseness=20] node[pos=0.5,left] {$\idarrow[1]$} (1);
%%     \draw (2) to [out=45,in=135,looseness=20] node[pos=0.5,right] {$\idarrow[2]$} (2);
%%     \draw (3) to [out=-45,in=-135,looseness=20] node[pos=0.5,right] {$\idarrow[3]$} (3);



%%   \end{tikzpicture}
%%   \caption{Rabi oscillations as a category}
