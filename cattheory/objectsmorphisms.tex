...
\mynameref{def:object} $i \in \mathrm{ob}(\cat{C})$ is called
...
Let $\cat{C}$ is a \mynameref{def:category}, the
\mynameref{def:object} $t \in \mathrm{ob}(\cat{C})$ is called
...
Let $\cat{C}$ is a category and $i, i' \in \mathrm{ob}(\cat{C})$ two
...
Let $\cat{C}$ is a category and $t, t' \in \mathrm{ob}(\cat{C})$ two
